...
本書若干做法承襲自他人的公開成果。本頁載明其出處、授權條件，以及本書所作的改動。

這些做法並非直接自原始專案移入本書。它們先納入作者自用的技能檔，經一段時間的實際使用與修改後，才寫入書稿。**中間那一層的判斷屬於作者，兩端不是。**
...
**本書以 CC BY-NC-SA 4.0 釋出**：得自由使用與改編，須註明出處，不得作商業用途，改編後散布須採相同授權。
...
上表第一列涉及五處，均出自同一來源，故單獨說明。

Academic Research Skills 為四套供研究者使用的技能：論文寫作（12 個角色、11 種模式）、論文審查（7 個角色、6 種模式）、文獻探索（13 個角色、8 種模式），另有一套 10 階段的全流程協調器。其規模遠大於本書所採用者。

**主要的改動不在刪減項目，而在使用者的設定。** 原作假設使用者能操作技能檔、能令多個角色並行、能管理階段之間的交接；本書假設讀者未曾寫過程式，且係一人於單一對話視窗中作業。因此各處改動方向一致：**將多角色系統降維為單人可循的流程。**
...
⚠️ 該文件另聲明其內容係為特定實驗室與作者自身的科研經歷所寫。本書所取者為逆向大綱與主張—證據對齊兩項通用寫作紀律，並已改寫為教育測量取向；**其於原始脈絡中係為另一領域而作**，讀者採用時宜自行折算。

---

## 六、查證方式

上表各筆授權均開啟該專案本身核實，未逕採作者技能檔中所載之宣告。兩項例外為 Creswell 之教科書（紙本，所引為框架結構，未逐頁閱讀）與 Anthropic 該文（官方部落格，已讀該頁）。

之所以強調此點，係因二手宣告曾三度出錯，且方向不同：

**其一。** 作者技能檔載明附錄 I「整合自 Supervisor-Skills `handbook 1.1`」。開啟原檔後，該份全文僅兩千餘位元組，內容為資料庫領域取向之「審稿人希望看到／討厭看到」各四條，原文載有 `within the DB community`，附錄 I 未採用其中任何一條。⚠️ 若未查證，本書將登記一項其實不存在的授權義務。

**其二。** 五維度評分同此。原作為 Higher／Faster／Stronger／Cheaper／Broader，十分制；本書為理論缺口／方法設計／樣本可行／測量嚴謹／實務貢獻，五分制。二者僅「以五個維度評分」此一形式相同，而形式不受著作權保護。

**其三，方向相反。** 作者原將四個中文潤稿專案均列為附錄 H 之上游，逐檔追溯後方知僅採用其中一項。同一次比對亦追出彭思達一項原本漏列。

多列將無謂放棄授權選項，漏列則有虧於人，二者皆曾發生。
