...
| # | 題目 | 過關的樣子 |
|---|---|---|
| **1** | 標出一筆文獻的驗證層級，並說出它**撐得住什麼、撐不住什麼** | 你講得出「只讀過摘要的這一筆，不能拿來撐方法學宣稱」這種話 |
| **2** | 你手上有一批**自己蒐集來的**文獻筆記，至少五篇 | 你自己找的，不是書上的範例 |
| **3** | 說出[第 2 章](../chapters/02-first-note.md)那三種編造型態 | 拼裝文獻、錯誤歸屬、虛假精確的數字——至少說得出兩種長什麼樣 |
| **4** | 你**曾經被一筆引用絆倒過** | 自己抓到的、指導教授抓到的、審查者抓到的，都算 |
| **5** | 你說得出「文獻存在」跟「你引的那句話正確」差在哪 | 你舉得出一個「論文是真的但說法被曲解」的例子 |

前兩題是硬前置——**這一章從頭到尾要你拿自己的東西來查**，沒有材料就只能看示範。第 4 題缺了不影響操作，但那一章的 5.1 會讀起來像在講別人的事。

**怎麼分流**

| 過幾題 | 這一章你該怎麼讀 |
|---|---|
| **5 題** | 直接做 5.2 那條端到端的流程，正文當參考。5.3 的綜整四步驟不可略 |
| **3–4 題** | 正常讀 |
| **2 題以下** | ⚠️ **先回[第 4 章](../chapters/04-research-question.md)**。缺第 1 題回 [4.5](../chapters/04-research-question.md#45-書目管理層讓機械工作歸機械)、缺第 2 題回 [4.3](../chapters/04-research-question.md#43-文獻工作的三個瓶頸)——**本章每一節都要你拿自己蒐集的文獻來跑** |
...
〔教你的 Claude 建這個〕**把查證做成一張逐項判準表。**

四步核實是最小版；真正在用的版本會長成一份**每一筆都要逐項回答、不可跳過**的清單。

**先自己寫。** 核心規則只有一條，而它是關於**怎麼回答**不是關於查什麼：每一筆引用都要逐項回答，**不可以整批說「都沒問題」**。

**寫完之後，用這三條檢查：**

| 檢查 | 為什麼 |
|---|---|
| 你列了幾項？ | 四項就夠開始：存在、作者名單逐欄、期刊與卷期頁、那句話在不在原文 |
| 有沒有要它**反過來問你**三件事 | 你過去被抓過哪一類問題／你的領域哪種來源最容易出錯／要不要加「必要性」那一項 |
| 有沒有寫「**不要幫我把清單補滿**」 | ⚠️ 少了它，你會拿到一份很完整的查核表——**而它防的是別人的錯，不是你的**。這份清單要從你自己的事故史長出來 |

**卡住超過十五分鐘再翻 → 〈範本對照冊〉第 10 則。**
...
〔教你的 Claude 建這個〕**把核實紀錄的格式固定下來，但把判斷留給自己。**

**先自己寫。** 這一份的關鍵不在格式，在**分工**：哪幾欄是它整理的，哪幾欄只能是你填的。

**寫完之後，用這三條檢查：**

| 檢查 | 為什麼 |
|---|---|
| 六欄你列齊了嗎 | 我寫的那句／出處位置／原文／比對結果／驗證層級／查核日期 |
| 你漏欄的時候，它要**指出來**還是**替你填** | 「指出來」是這份檔案的用處，「替你填」會讓紀錄失去意義 |
| **有沒有一行 ⛔ 擋住「比對結果」與「出處位置」** | ⚠️ **那一行是這個檔案唯一重要的地方。它替你填的比對結果，會看起來跟你自己填的一模一樣**，而你三個月後分不出哪幾筆是真的查過的 |

**那兩欄你沒填，就是你還沒做完，不是要它補。**

**卡住超過十五分鐘再翻 → 〈範本對照冊〉第 11 則。**
...
〔教你的 Claude 建這個〕**把書目層的機械工作固定成一個檔案。**

**先自己寫。** 這一份處理的是**純機械**的工作：補 DOI、抓正確卷期頁、找出重複條目。機械工作可以交出去——但它仍然需要三條規則，而那三條都跟「它不確定的時候怎麼辦」有關。

**寫完之後，用這三條檢查：**

| 檢查 | 為什麼 |
|---|---|
| DOI 的格式你寫死了嗎 | 一律完整網址，不要只寫前綴——你是照著清單逐筆開連結的那個人 |
| 查不到 DOI 的時候，它該**標「查無」**還是**生一個** | 不寫這一條，你會拿到格式完美但不存在的 DOI |
| **回報時「查到的」與「推測的」有沒有分開** | ⚠️ **這是最重要的一條。沒有它，這個技能會安靜地把推測當成查到的東西交給你**——而書目正是你最不會逐筆複查的地方 |

⚠️ 另外記得反過來問它一件事：**你的書目庫裡有幾種來源型態**（期刊、專書章節、學位論文、研討會、政府報告），各自需要哪些欄位。少了這一問，它會用期刊論文的欄位套所有東西。

**卡住超過十五分鐘再翻 → 〈範本對照冊〉第 12 則。**
...
〔教你的 Claude 建這個〕把「當反方」固定成一個檔案，**因為臨時想的反方會太客氣**。

**先自己寫。** 這一份要交代的是：它拿到什麼、它只准做什麼、以及它固定要回答哪四件事。

⚠️ 難的是第二項。反方最容易失效的方式不是挑得不夠狠，是**順手替你改寫主張**—— 那樣你會拿到一個更好的版本，卻不知道原本那個錯在哪。

**寫完之後，用這三條檢查：**

| 檢查 | 為什麼 |
|---|---|
| 有沒有寫「**只有挑毛病，不准替我改寫主張**」 | 見上。這是這份檔案的界線 |
| 你的四個固定問題，有沒有一題在問**只有一篇文獻在撐** | 那是 5.3 講的單一來源問題，也是最難自己看出來的一種 |
| 最後有沒有要它**分開列「有把握的」與「在猜的」** | 反方的猜測跟反方的證據混在一起，你會被一個猜測嚇到而改掉一句站得住的主張 |

**卡住超過十五分鐘再翻 → 〈範本對照冊〉第 13 則。**
...
⚠️ **這一章的那一格幾乎一定填得出來，而那正是重點。** 5.1 的四步核實從頭到尾是你在做——解析 DOI、在資料庫交叉比對、打開 PDF 找那一句話。**這一節裡工具的角色是那些錯誤的來源，不是解法。**

所以這一格填不出日期的話，要問的不是「我是不是太依賴它」，而是**「我上次查引用，是不是把『請它幫我檢查』當成查過了」**——那是本章迷思二。
