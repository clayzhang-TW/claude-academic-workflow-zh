...
一位博士班二年級的學生，過去半年幾乎每天都在用 AI。查概念、改句子、討論研究設計、問統計怎麼跑。她覺得很有用。

然後她要寫研究計畫的方法章，想找回三個月前那次很有幫助的討論——那次她跟 AI 來回了一個多小時，最後釐清了為什麼她的設計不能用某個分析方法。她記得結論，但記不得理由。

她翻對話紀錄。找到了，但那是一個八十多則往返的長串，關鍵那幾句夾在中間，而她已經想不起來是哪幾句。她讀了二十分鐘，放棄，決定重問一次。

重問的結果不一樣。**不是它變笨了，是它不知道當初那一個小時裡發生過什麼。**

她那半年的使用，回報全部發生在當下：每一次都省了一點時間，每一次都覺得有用。 **但那些回報沒有累積。**半年之後，她手上有的東西跟半年前一樣多。
...
> **〔課堂〕開場 8 分鐘** 先讓全班做上面那個 2 分鐘的〔動手一下〕，**不要先講任何東西**。做完問一句：「有沒有人現在還找得到自己三個月前跟 AI 討論過什麼？」多數人找不到，或找得到但不想找。那個沉默就是本章的起點。 ⚠️ 不要把這件事導向「所以要好好做筆記」的道德勸說。重點不是勤勞，是**現在的工具讓你的努力留不下來**，而那是可以修的。
...
完整的提示詞寫法有七個元素，那是第 3 章的內容。**現在兩個就夠**，而且多數人的問題從來不是元素不夠多，是連這兩個都沒給。
...
〔教你的 Claude 建這個〕**這一節的做法可以固定下來，而且現在就可以。** 把下面這段貼給它：「我剛才用『動作＋背景』的寫法要你做了一件事。請幫我寫成一份說明檔，讓我下次直接叫它就好。檔案裡要有三段：（一）我是誰、我在做什麼領域的研究；（二）我最常要你做的三種診斷；（三）你**不要**做的事——不要改寫、不要補我沒給的資料。只寫這三段，不要幫我加別的。」第一版寫短一點。**它會長大，但不是今天。** 完整的做法在第 3 章。

### 第 3 步才是這一節的重點

第 3 步是「把結果存成一個檔案」。這一步看起來最無聊，卻是整本書的地基。

你剛才那個對話，如果你什麼都不做，它三個月後對你的價值是零—— 不是因為它沒被保存，是因為**你找不到它，而且找到了也讀不動**。
...
所以那句「它應該知道我在做什麼」永遠不成立，除非**你把那些東西放進它讀得到的地方**。

### 於是問題變成：放在哪裡

而這個問題有一個你可能沒想到的答案：**那個地方不是為 AI 建的，是為你自己建的。**
...
〔圖 1-1〕**兩種記憶**。左邊屬於專案，做完就凍結；右邊跟著你走，跨專案累積。兩者都不是為 AI 建的，**但建起來就是最好的脈絡**。
...
檔案系統要你**替每份東西選一個位置**。但研究裡最有價值的東西通常同時屬於好幾個位置：一篇關於「教師 AI 焦慮」的論文，它同時是你的量表開發專案的材料、是「科技接受」這個概念的實例、也是你下學期課程的教材。

放進哪個資料夾？**放哪裡都會漏掉另外兩個用途。**

知識庫的作法不同：檔案放哪裡不重要，重要的是**它連到哪些東西**。同一篇筆記可以同時被三個地方引用，而你從任何一邊都找得到它。
...
**但這條界線要看清楚**：交出去的是**整理**，不是**判斷**。它可以幫你把二十篇筆記照主題分堆，**不能替你決定哪一堆才是你的論點**—— 那件事第 5 章會處理，而且會明說它幫不上。

〔補充〕**這個分工也解釋了為什麼要用純文字。** 一份 `.md` 檔對 AI 來說是最省事的輸入：沒有專有格式要解析、沒有版面雜訊、內容進去多少就是多少。而「省事」在這裡有實際意義——**同樣一批筆記，格式乾淨的那份能讓它一次讀進更多**。第 3 章談到對話容量上限時，你會再遇到這件事。
...
〔補充〕**網頁可以一鍵剪進來。** Obsidian 有一個瀏覽器擴充功能可以把整篇網頁存成筆記，並自動帶上標題、網址、日期。安裝步驟在附錄 A——這裡只要知道**有這件事**，因為它會改變你對「這個庫要裝什麼」的想像：**不只是論文，任何你日後要回頭找的東西。**
...
**這一項對中文研究者的價值遠高於英文使用者**，理由很具體：你的同一個構念至少有中文名、英文名、縮寫三種寫法，而且**你在不同場合會用不同的那一種**。沒有別名，你會建出三則講同一件事的筆記，然後三個月後不知道該信哪一則。
...
完整的做法在第 7 章（那時候你手上會有真的資料）。 **第一天只要做一件事：建 vault 的時候，替敏感材料另開一個資料夾，而且不要把它放在同步範圍裡。**

〔補充〕**版本歷史值得花一點心思。** 同步方案（官方服務、一般雲端硬碟、版本控制工具）差別最大的不是價錢，是**你改壞了能不能救回來**。研究筆記最痛的失誤不是弄丟檔案，是**在一份還在用的筆記上覆蓋掉舊內容而沒發現**。選同步方案時，把「有沒有版本歷史」排在「同不同步得快」前面。
...
⚠️ **沒寫理由的日誌等於沒寫。** 因為未來的你要做的事是**推翻過去的決定**，而你推翻的不是決定本身，是理由。如果當初的理由是「樣本數不足」，而現在你補了資料，那則日誌會直接告訴你可以回頭。**只寫決定不寫理由，你就得從頭再想一次為什麼。**

〔補充〕**這一欄也是你日後回答口試問題的稿子。** 「你為什麼用這個方法？」這種問題，答得出來跟答得好，差別在於你有沒有當時的理由。事後重建的理由聽起來永遠比較整齊，而**整齊往往代表你已經忘了真正的原因**。
...
很多人以為 Chat、Projects、Cowork、Code 是四個不同的產品。不是。 **後面跑的是同一個模型**，差別在它看得到什麼、能動什麼：
...
從左到右，它看得到的越多、能動的越多、自動化程度越高。

**自動化程度高不是「比較高級」，是「出事的傷害範圍比較大」。** 這一句是本書後面所有委派判斷的起點。
...
這是本章的搭建與後面六章之間的接口——第 2 章的文獻管線，終點就是它。
...
現在不用學怎麼查，第 2 章會教。現在你需要記得的是：**這件事是你的**。
...
開場那位找不回三個月前討論的博士生，後來做了一件很小的事：她在每次跟 AI 討論完研究決定之後，花三十秒寫一則決策日誌。

三個月後她再遇到同樣的狀況——要回頭找一個決定的理由。這次她打開日誌，第一行就寫著：**「改用穩健估計法：資料非常態，原方法的檢定量會膨脹。」**

注意那一行的**理由欄**。日期和決定她其實記得住，**忘掉的永遠是理由**。而口試委員問的、未來的她要推翻的，也永遠是理由。
...
第 3 章會講對話為什麼有這個上限。**現在你只需要知道：東西多的時候，換入口，不是換講法。**
...
**做對了會看到**：它列出的是**具體的句子與位置**，不是「整體來說可以再加強」這種話。如果拿到的是後者，回去看你的第 2 步——多半是動作寫得不夠具體。
...
**做對了會看到**：vault 裡至少有三個檔案，而且**你說得出每一個是給誰用的**。
...
*為什麼錯*：用得久跟留得下來是兩件事。開場那位博士生用了半年，而她手上的東西跟半年前一樣多。
...
那位博士生的問題不是她不夠努力，也不是她提示詞寫得不好。

**是她的努力沒有落點。** 每一次對話都有價值，但價值隨著視窗關閉一起消失。

你現在有落點了。它現在還很空——一個 vault、三個檔案、一則日誌。但從下一章開始，**每一章都會往裡面放東西**，而且放進去的東西會互相接上。

![圖 1-2 你在整條工作流的哪裡](../resources/figures/fig1-2_工作流地圖_W1.svg)
...
**那是刻意安排的。** 第 2 章會讓你親手抓到它編造的東西—— 不是為了讓你不信任它，是為了讓你知道**它出錯的時候長什麼樣子**。那是本書所有工作流設計的起點。
...
- **第 2 章**：文獻筆記管線。1.3 建的 vault 就是它的終點
- **第 3 章**：提示詞七元素、上下文視窗、技能檔。1.1 給的兩元素在那裡展開成七個
- **第 4 章**：委派判斷與 4D 框架。1.4 那句「自動化程度越高、傷害範圍越大」在那裡變成一個判準
- **附錄 A**：安裝與帳號設定（介面步驟集中在那裡，因為它最容易過期）
