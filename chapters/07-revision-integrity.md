...
- 拿出一份**你自己寫的**草稿，哪怕只有一節（[6.3](../chapters/06-analysis-writing.md#63-寫作模式鏈)）
- 說出統計協作那條責任邊界上，哪幾個判斷不能交出去（[6.1](../chapters/06-analysis-writing.md#61-統計輔助的責任邊界)）
...
![圖 7-1 三輪抓的不是同一種東西](../resources/figures/fig7-1_三輪抓的不是同一種東西.svg)

〔圖 7-1〕三格裡的單位一格比一格小。 **順序不能換的理由就在這裡**——粗的那一輪會把細的那一輪剛整理好的東西推翻。
...
這就是副駕駛。它接手的是右邊那一欄；右邊空出來的時間要回到左邊，不是回到它手上。
...
自審在工作流上是獨立的一站（本章末的圖 7-10，「送出」那一區亮起來的第二格），不是修稿順手做完的收尾動作。一站要成立，得說得出三件事。
...
![圖 7-2 要找的不是誰寫得最兇，是哪一段被指了三次](../resources/figures/fig7-2_重疊處才是訊號.svg)

〔圖 7-2〕五個角色各自獨立跑完，**看的是線的終點怎麼分布**。五份都跑完之後才並排看——你要找的是那個被指了不只一次的地方。
...
![圖 7-3 同一批證據撐得起相反的兩句](../resources/figures/fig7-3_同一批證據撐得起相反的兩句.svg)

〔圖 7-3〕左右兩欄的證據都指得出來，差別只在**線往哪裡連**。右欄那三條線上下都連得到，這代表你手上的東西同時撐得起兩句相反的話。
...
第二類也有自己的層次，而這件事 [4.5](../chapters/04-research-question.md#45-書目管理層讓機械工作歸機械) 定義過、[5.1](../chapters/05-verification.md#51-引用幻覺與四步核實) 的核實紀錄裡有一整欄就是它：**你讀到全文、只讀了摘要、還是只看到書目，決定那個引用撐得起多重的主張。** 只看過摘要就寫「Smith 證實了 X」，那是把第二類當第一類用——跟上面那個錯是同一種，只是換了一個對象。
...
![圖 7-4 重複題與對照題測的不是同一件事](../resources/figures/fig7-4_重複題與對照題測的不是同一件事.svg)

〔圖 7-4〕上下兩排的形狀不同，因為它們問的不是同一個問題。上排看的是**時間**（同一個東西，隔開再問），下排看的是**差異**（兩個東西，只差一格）。
...
重複題與對照題的分工圖 7-4 講完了。這裡補兩件圖上沒有的。
...
第 4、5 條是第一階段的重點，其餘幾條做錯了你看得出來，這兩條做錯了你看不出來。第 7 條的後半是第二階段的重點：**工具算，你驗收**（[6.1](../chapters/06-analysis-writing.md#61-統計輔助的責任邊界)）。
### 怎麼做到看不見自己上次的答案
...
這一步的預設做法是**讓它一條一條問你**，不是你想不出來才找它幫忙：「這條意見的核心訴求是什麼？你同意嗎？為什麼？」一次一條，答完再問下一條。回應信裡最難寫的是讓步範圍——你打算讓到哪裡、哪裡不讓、不讓的理由是什麼——而那三件事被問出來的品質，比自己坐著想出來的高。最終的判斷仍然是你的（做法在[第 6 章](../chapters/06-analysis-writing.md) [6.3](../chapters/06-analysis-writing.md#63-寫作模式鏈)）。
...
![圖 7-5 關卡不放在每一步之間](../resources/figures/fig7-5_關卡不在每一步之間.svg)

〔圖 7-5〕三道線沒有平均分布。**它們全部擠在同一個縫隙裡**，因為那裡是一動筆就收不回來的地方。
...
〔補充〕這四步在成熟工作流裡會多三個關卡。 我自己的審稿回應工具在流程中途設了三個強制停點：意見分類之後問每條要怎麼處置、補跑統計**之前**問這個分析要不要跑、動筆之前問讓步範圍到哪裡。三個關卡落在同一種位置：這一步做下去就難回頭。其中第二個帶著一條硬規則。補跑的統計若結果不利於原結論，不得靜默不報。那條規則不是禮貌，是把誠信寫成機制。你的版本只需要一個關卡：找出流程裡「做錯了很難回頭」的那一步，在它前面插一個問句。第三個關卡（動筆之前問讓步範圍）其實就是 [6.3](../chapters/06-analysis-writing.md#63-寫作模式鏈) 那個做法落在審稿回應上的樣子——一次問一件，答完才准往下寫。關卡太多，你會養成整批略過的習慣，那時候等於一道都沒有。決策卡的四欄格式見[附錄 F](../appendix/F-skill-library.md) 的模式④。
...
![圖 7-6 改一個定義，四個方向會跟著動](../resources/figures/fig7-6_爆炸半徑.svg)

〔圖 7-6〕中間那一格有編號，你一定會改。**四個方向一個編號都沒有。**
...
![圖 7-7 判準就是那條線](../resources/figures/fig7-7_判準即分界線.svg)

〔圖 7-7〕**判準就是中間那條線。** 七章的每一件事，都被同一個問題分到左邊或右邊。左欄七格各不相同；右欄七格的性質完全一樣：機械、形式、可重複。所以第 7 章不再增加新的判準。它只是把前六章的位置並排放，讓你看見它們是同一條線。這不是七個獨立的提醒，是**同一個判準的七次應用**。
...
![圖 7-8 關卡都長在同一條邊界上](../resources/figures/fig7-8_內外分界.svg)

〔圖 7-8〕**六道關卡全部長在同一條線上**。那條線就是「還在你手上」與「已經出去了」的分界。左邊那一欄從頭貫穿到尾：它不在任何一站上，但每一站都回來取用它。這張圖跟圖 7-7 是同一條線的兩個切面：**7-7 說判準是一條線，7-8 說那條線的實體位置在哪裡。**
...
![圖 7-9 三層起步，每層單獨成立](../resources/figures/fig7-9_三層起步.svg)
...
這也是 [6.3](../chapters/06-analysis-writing.md#63-寫作模式鏈) 那個做法真正的收益。它省的不是寫作時間，是**那一天**。
...
![圖 7-10 你在整條工作流的哪裡](../resources/figures/fig7-10_工作流地圖_W7.svg)

〔圖 7-10〕亮起來的是這一章教的位置，淡掉的是你在別章會站上去的地方。最後三站的共同點只有一個：**產出即將離開你的手。** 那正是所有驗收關卡的位置（圖 7-8）。
