...
自審在工作流上是獨立的一站（本章末的圖 7-8，「送出」那一區亮起來的第二格），不是修稿順手做完的收尾動作。一站要成立，得說得出三件事。
...
![圖 7-3 關卡不放在每一步之間](../resources/figures/fig7-3_關卡不在每一步之間.svg)

〔圖 7-3〕三道線沒有平均分布。**它們全部擠在同一個縫隙裡**，因為那裡是一動筆就收不回來的地方。
...
![圖 7-4 改一個定義，四個方向會跟著動](../resources/figures/fig7-4_爆炸半徑.svg)

〔圖 7-4〕中間那一格有編號，你一定會改。**四個方向一個編號都沒有。**
...
![圖 7-5 判準就是那條線](../resources/figures/fig7-5_判準即分界線.svg)

〔圖 7-5〕**判準就是中間那條線。** 七章的每一件事，都被同一個問題分到左邊或右邊。左欄七格各不相同；右欄七格的性質完全一樣：機械、形式、可重複。所以第 7 章不再增加新的判準。它只是把前六章的位置並排放，讓你看見它們是同一條線。這不是七個獨立的提醒，是**同一個判準的七次應用**。
...
![圖 7-6 關卡都長在同一條邊界上](../resources/figures/fig7-6_內外分界.svg)

〔圖 7-6〕**六道關卡全部長在同一條線上**。那條線就是「還在你手上」與「已經出去了」的分界。左邊那一欄從頭貫穿到尾：它不在任何一站上，但每一站都回來取用它。這張圖跟圖 7-5 是同一條線的兩個切面：**7-5 說判準是一條線，7-6 說那條線的實體位置在哪裡。**
...
![圖 7-7 三層起步，每層單獨成立](../resources/figures/fig7-7_三層起步.svg)
...
![圖 7-8 你在整條工作流的哪裡](../resources/figures/fig7-8_工作流地圖_W7.svg)

〔圖 7-8〕亮起來的是這一章教的位置，淡掉的是你在別章會站上去的地方。最後三站的共同點只有一個：**產出即將離開你的手。** 那正是所有驗收關卡的位置（圖 7-6）。
