...
- 說出上一章跑那條管線時，**哪一步花的時間最多**（2.1）。本章從那個地方開始
- 講出你在 2.2 **親手抓到的那一筆**它編出來的東西是什麼

第二條特別重要。沒有真的抓到過的人，本章講「限制要寫進檔案」那一段會讀起來像常識，而它不是常識——它是吃過虧才學得到的東西。

**開始前請準備**：第 2 章跑管線時你實際用過的那幾段提示詞。沒存下來的話，現在回頭把它們複製出來——本章從第一頁就要用。

---

## 章前導言

上一章你跑了一次完整的管線：轉檔、七欄筆記、糾正迴圈、查證、存檔。
...
> **〔動手一下〕2 分鐘** 打開你第 2 章用過的那段七欄提示詞，把它從頭讀一遍，然後問自己：**如果現在憑記憶重打一次，我會漏掉哪一句？** 把那一句圈起來。這一章就是為了那一句而存在的。
...
而且提示詞的完整寫法**在這一章才展開**，理由跟第 2 章一樣：你要把它們寫進一個會用很久的檔案，**這時候「該寫哪些欄位」才是一個真問題**。
...
看的時候請注意每格最後那一行。它不是說五段各該寫多少字，是說**你今天該寫多少**：①寫滿，②一句話，③三到五行，④兩行，⑤**可以完全空著**。第五段之所以留白，不是因為它不重要——**它是全檔最有價值的一段**，但它只能靠你自己被絆到才長得出來，抄不來也想不出來。理由在 3.5。

> **〔動手一下〕8 分鐘** 現在就開一個純文字檔，**只填〈使用者背景〉那一段**，其他先空著。存進第 1 章那個 vault。 **不要一開始就想寫完整。** 一個只有背景段的技能，比一個你抄來但看不懂的完整技能有用得多——理由在 3.5。
...
所以**安裝是最佳化，不是前置條件**。本章與後面每一章的實作都不依賴它。（安裝步驟在附錄 A。）
...
**這兩件事是同一件事的兩端，而且會撞在一起。** 你最容易忘的那一句是限制；而限制在七元素裡沒有自己的位置，只能寫進 Context——Context 又是 3.3 那份清單裡最先被擠掉的一層。**於是你最常忘的那一句，剛好也是它最常不遵守的那一句。**

這正是把它寫成檔案最大的價值——**限制寫進檔案之後，兩邊都不必再記得它。**

**（二）迭代不是失敗，是流程的一部分。**

這裡要講清楚一件事，免得誤會：**本書其實一直在講迭代**——第 6 章講工具怎麼跟著研究長、第 7 章講修稿三輪、附錄 F 專門有一節講「迴圈要有出口」。
...
下面是我自己每天在用的一份技能檔的**開頭三十行**，原樣貼出，只刪掉與本書無關的段落。它管的是「引用進草稿之前要先過一關」——也就是第 5 章整章在講的那件事。
...
4. **它的版本號是 1.8.0**（此處已刪去），代表改過十幾輪。第一版沒有「隱式觸發」那一條——那一條是**在我又一次差點把沒查證的引用寫進草稿之後才補上去的**。**技能是長出來的，不是設計出來的**，3.5 整節在講這件事。
...
問一個術語的定義，Role 和 Task 就夠了（第 1 章給的那兩個）。要它評估你的研究設計，七個都值得寫。
...
第 1 章講過**脈絡**：它生成下一個字時實際看得到的所有文字。那個空間有上限，叫**上下文視窗**。
...
〔補充〕**這一點跟第 2 章那條原則是同一件事。** 它的自信不帶正確性資訊—— 同樣地，**它答得順不代表它看得到你要的東西**。

![圖 3-2 為什麼對話會愈用愈鈍](../resources/figures/fig3-2_上下文視窗.svg)

〔圖 3-2〕注意兩件事。**第一，視窗裡越到後面，佔位置的主要是它自己的回答**，不是你講的話——你只打了兩行，它回了六百字。**第二，掉出去的是最早那幾句**，而最早那幾句通常正是你交代的限制與背景。
...
![圖 3-3 兩條軸，四樣東西各站一個位置](../resources/figures/fig3-3_兩軸.svg)

〔圖 3-3〕**位置有意義。** 越往右它自主性越高，越往上它越不需要你叫。接頭不在這兩條軸上——它不移動任何一格，它把整張圖**能碰到的範圍**變大。
...
| **本地覆寫版** | 只有你這台機器要遵守的（路徑、暫時的例外） | **不進版控** |
...
〔動手一下〕**5 分鐘。** 翻你最近三次跟它的對話，找出**你重複打過的那幾句背景說明**。那幾句就是你規則檔的第一版，不必再想別的。
...
2. **每一條附一句為什麼。** 這跟第 1 章決策日誌的「理由欄」是同一件事：有理由，它才判斷得出**沒被明文寫到的相似情況**該怎麼辦。
...
**還有一個更根本的限制**：規則檔是**請求**，不是**強制**。你寫「每次都要先問我」，它多數時候會照做，但不是每一次。 **真正非做不可的事，不要只寫在規則檔裡**——把它變成一道你自己會執行的關卡（3.2 那份真實技能檔裡的兩條「強制觸發」就是這個做法），或是**一個你自己在流程裡會停下來的動作**。 **拜託沒有用，關卡才有用。**
...
⚠️ **這些細節幾乎全部出自當事人自己貼出的截圖與影片**，記者當時向廠商求證未獲回應，事故資料庫通篇用「據稱」。所以照本書第 5 章的規矩，上面那段只能當成當事人自述，不能當成查證過的事實。**下面這一段才是有分量的部分，而且它不依賴上面任何一項。**
...
換句話說：一家把這件事追到底、而且要付錢賠償的公司，最後的答案不是**更好的請求**，是**把東西移到請求構不到的地方**。
...
前面所有東西都在講「怎麼跟它講」。接頭講的是另一件事：**它伸得到哪裡。**

預設狀態下它只看得到你貼進去的東西，加上你指給它的那個資料夾。接頭是一種設定，裝上去之後它可以直接讀寫外部程式——你的書目軟體、你的行事曆、某個資料庫的檢索介面。裝一次，之後每次對話都用得到。

一句話分工：

> **接頭決定「碰得到什麼」，技能決定「碰到之後怎麼做」，提示詞只是按下開始。**

三件實務上一定會踩到的事，先說在前面：

1. **裝完要把程式完全關掉再開，不是開一個新對話。** 接頭只在程式啟動時載入。改完設定工具沒出現，多半不是你設定寫錯，是它還沒重讀。
2. **金鑰不要寫進技能檔。** 技能檔會被你分享出去、會進版本紀錄，金鑰一旦混進去，事後很難清乾淨。金鑰放它自己的設定檔。
3. **它有回應，不代表接通了。** 有些功能是直接讀本機檔案的，外部程式根本沒開，它照樣回你東西。**要驗接通，挑一個非連線不可的功能去試。**

〔補充〕**接頭的門檻比聽起來低，但它不是本書的必修。** 全書的實作沒有一項需要接頭—— 文獻管線、技能檔、查證流程，用到的只有「貼檔案」跟「指資料夾」。接頭讓其中幾步變快，**不讓任何一步從不可能變成可能**。這是刻意的安排：綁在特定接頭上的教材，會跟著那個接頭一起過期。
...
![圖 3-4 一次對話裡，四樣東西什麼時候進場](../resources/figures/fig3-4_載入時機.svg)

〔圖 3-4〕同樣四樣東西，換一條軸看：**一次對話的時間軸。** 規則檔在你做任何事之前就已經載入了，接頭則要等到它真的需要碰外部程式。 **越早進場的，你越容易忘記它還在**——這正是規則檔要短的原因。
...
- **接頭**決定它碰得到什麼；**它有回應不等於接通了**，要用非連線不可的功能去驗
...
三個月之後，你的檔案會有一節寫著各種「不要做什麼」—— **那一節就是你的事故史，也是整份檔案最有價值的部分。**
...
〔補充〕**再一個不要直接抄的理由，這個跟品質無關，跟安全有關。** 別人分享的技能有時候不只是文字——它可能附帶**會被實際執行的腳本**。你把它放進工作環境，等於同意那些腳本在你的電腦上跑， **而它讀得到的範圍就是你指給它的那個資料夾**（第 1 章講過那條線）。所以拿到別人的技能包，**先把裡面的檔案打開看一遍**；看不懂的那幾個，可以請它先幫你解釋每個檔案在做什麼、有沒有動到檔案系統或網路。 **看不懂又不敢問的，就不要裝。**
...
〔補充〕**這一招跟本書第 4 章的執行前追問是同一個動作的兩種用法。** 一個問「這次要做的事，你需要我確認什麼」，一個問「你需要我怎麼講」。
...
**刪的時候要留紀錄。** 在檔案底部寫一行「某月某日刪掉某條，理由是……」—— 理由跟第 1 章的決策日誌一樣：**你未來要推翻的不是那條規則，是當初的理由。**
...
而更重要的是第二件事——**你已經不需要記得它了**。這就是這一章全部的價值：不是讓你變得更會下指令，是讓你**不必每次都重新變得會下指令**。

![圖 3-5 你在整條工作流的哪裡](../resources/figures/fig3-5_工作流地圖_W3.svg)

〔圖 3-5〕**亮起來的是這一章教的位置，淡掉的是你在別章會站上去的地方。** 技能檔亮在**貫穿層**，不在任何一站上。這是刻意的：**它不是流程裡的一步，它是讓每一步都變快的那一層。**
...
- **Lee, D., & Palmer, E. (2025).** Prompt engineering in higher education: a systematic review to help inform curricula. *International Journal of Educational Technology in Higher Education, 22*(1). [https://doi.org/10.1186/s41239-025-00503-7](https://doi.org/10.1186/s41239-025-00503-7) 33 篇的系統性回顧，本節 3.2 的「三條共通點」與「綜整七步」都出自它。**開放取用，值得自己讀一次。** **兩個限制要一起知道**：（一）該領域**沒有共通的成功指標**，多數研究是主觀判斷；（二）有人類參與者的研究**樣本都很小**（九項合計 224 人，最小三人）。 **驗證層級：全文讀過。**
...
- **第 1 章 1.1**：兩元素最小版。本章 3.2 把它展開成七個
- **第 1 章 1.4**：四個入口。那句「自動化程度越高，傷害範圍越大」在本章 3.4 變成判準
- **第 2 章**：你要固定下來的那條管線
- **第 4 章**：執行前追問。3.5 那句「我現在該用什麼提示詞」是它的另一種用法
- **附錄 F**：十二個成熟工具反覆出現的設計。**現在不要照著加**—— 那份附錄的第一條就是「第一週只寫〈使用者背景〉」
- **附錄 A**：安裝與帳號設定
