...
- 說出上一章跑那條流程時，**哪一步花的時間最多**（2.1）。本章從那個地方開始
- 講出你在 2.2 **親手抓到的那一筆**它編出來的東西是什麼

第二條特別重要。沒有真的抓到過的人，本章講「限制要寫進檔案」那一段會讀起來像常識，而它不是常識——它是吃過虧才學得到的東西。

**開始前請準備**：第 2 章跑流程時你實際用過的那幾段提示詞。沒存下來的話，現在回頭把它們複製出來——本章從第一頁就要用。
...
〔補充〕**接頭的門檻比聽起來低，但它不是本書的必修。** 全書的實作沒有一項需要接頭—— 文獻流程、技能檔、查證流程，用到的只有「貼檔案」跟「指資料夾」。接頭讓其中幾步變快，**不讓任何一步從不可能變成可能**。這是刻意的安排：綁在特定接頭上的教材，會跟著那個接頭一起過期。
