...
**五題，三分鐘。** 每一題都問你**做得出來**或**說得出來**，不問你覺得懂不懂。

| # | 題目 | 過關的樣子 |
|---|---|---|
| **1** | 說出上一章跑流程時**哪一步花的時間最多** | 你講得出一個具體的步驟，不是「都差不多」 |
| **2** | 講出你在 [2.2](../chapters/02-first-note.md#22-現在讓它騙你一次) **親手抓到的那一筆**是什麼 | 你說得出它編了什麼、你怎麼發現的 |
| **3** | 你手上有[第 2 章](../chapters/02-first-note.md)**實際用過的那幾段提示詞** | 存下來了，不是憑印象重打 |
| **4** | 說出[第 2 章](../chapters/02-first-note.md)那道閘門的**五個問題** | 至少講得出「值不值得完整跑一次」那一題與它的三種答案 |
| **5** | 你已經被同一件事**重複交代過三次以上** | 想得出是哪一句話 |

前三題是硬前置。第 4、5 題缺了還走得動，但第 5 題缺的人要注意—— **這一章的觸發點就是那個重複**，沒有它，本章會讀起來像在講別人的問題。

**怎麼分流**

| 過幾題 | 這一章你該怎麼讀 |
|---|---|
| **5 題** | 正文略讀。直接做 3.1 的〔動手一下〕，然後跳 3.5 |
| **3–4 題** | 正常讀 |
| **2 題以下** | **先回[第 2 章](../chapters/02-first-note.md)**。缺第 2 題最要緊——**沒有真的抓到過的人，本章講「限制要寫進檔案」會讀起來像常識，而它不是常識，它是吃過虧才學得到的東西** |
...
**還會有第六段，但不是今天。** 這五段管的是「它怎麼幫你做這件事」。還缺一段管**「你什麼時候該少用它」**——那一段叫〈退場〉，做法在 3.5。現在先不寫，因為你還不知道自己會不會依賴它。⚠️ **但要記得有這回事**：從 3.5 起，這本書要求你建的每一個檔案都以那一段結尾。
...
〔教你的 Claude 建這個〕**把你的檔案補到七欄。**

**先自己寫。** 這一份不是要它幫你填，是要它**逐項問你**。所以你要交代的是：七項各是什麼、以及它每缺一項該做什麼。

**寫完之後，用這三條檢查：**

| 檢查 | 為什麼 |
|---|---|
| 你寫的是「幫我補齊」還是「**每缺一項就問我一個具體問題，不要幫我填**」 | ⚠️ 前者你會拿到一份它猜出來的檔案，而它猜得很像你會寫的東西 |
| 有沒有另外問「**哪些東西是你不准動的**」 | 那是七欄裡沒有的第八件事——限制。回想章前導言你圈起來的那一句 |
| 有沒有問「**上次哪裡不滿意**」 | 那是迭代的入口。這兩問對應的正是七元素相對於回顧綜整缺的兩項 |

**卡住超過十五分鐘再翻 → 〈範本對照冊〉第 5 則。**
...
〔教你的 Claude 建這個〕想把這一節變成你自己的規則檔。

**先自己寫。** 規則檔跟技能檔不同：技能檔管「怎麼做這件事」，規則檔管「這個專案裡什麼不能違反」。所以它應該由**五個問題**開始，而不是由你單方面列條文。

**寫完之後，用這三條檢查：**

| 檢查 | 為什麼 |
|---|---|
| 你是要它**先問你五件事**，還是直接產出？ | 直接產出的規則檔，寫的是它以為你的專案長什麼樣 |
| 每一條都**具體到可以判斷有沒有違反**嗎 | 「要嚴謹」判斷不了。**寫不具體的那幾條，先不要寫進去**——它們會稀釋看得懂的那幾條 |
| 第一版有幾條？ | **五到八條，一頁之內。** 不要一開始就寫成手冊 |

**規則檔的長度會自己長，而長出來的每一條都應該對應到一次你真的被絆到的經驗。**

**卡住超過十五分鐘再翻 → 〈範本對照冊〉第 6 則。**
...
〔教你的 Claude 建這個〕**讓它幫你維護版本紀錄。**

**先自己寫。** 這一份要規定的是「每次改這個檔案的時候，你該做哪三件事」。三件裡有兩件發生在**動手之前**。

**寫完之後，用這三條檢查：**

| 檢查 | 為什麼 |
|---|---|
| 改之前，它要不要先問你「**這次是哪一個實際發生的問題觸發的**」 | 沒有這一問，你會加進一堆想像出來的規則——3.5 整節在講這件事 |
| 變更紀錄有沒有「**為什麼**」那一欄 | 只記「改了什麼」，你日後推翻不了它。跟[第 1 章](../chapters/01-setup.md)的決策日誌同一個道理 |
| 新規則跟既有的某一條**衝突時**，它要先講還是直接改 | ⚠️ **這是三條裡最有用的一條。衝突的規則不會報錯，只會讓它的行為變得看不懂**，而你要花很久才找得到原因 |

**卡住超過十五分鐘再翻 → 〈範本對照冊〉第 7 則。**

### 也要寫退場

上面講的都是**工具層**的事：加一條規則、刪一條規則，讓這個檔案更好用。

這一小節講另一層：**這個檔案，你什麼時候該少用它。**

兩者不是同一件事。刪規則問的是「這條規則還有用嗎」；退場問的是 **「我還需要這個檔案幫我做這件事嗎」**。一個技能檔可以每一條規則都很好用，而你仍然該減少開它的次數。

#### 先講清楚它從哪裡來

「鷹架」這個說法有明確的來源。van de Pol 等人（2010）回顧了十年的鷹架研究，把它拆成**三個關鍵成分**：**因應調整**（contingency）、**逐步撤除**（fading）、 **責任移轉**（transfer of responsibility）。

⚠️ **那篇回顧還講了一句更重要的話：用了支持策略，不自動等於發生了鷹架。** 一段互動要算鷹架，得同時具備那三個成分。

把這三個成分拿來對照你手上的工具，落差很清楚：

| 成分 | 它是什麼 | 你的工具做得到嗎 |
|---|---|---|
| **因應調整** | 支持的量要隨你**當下的程度**調整。那篇回顧寫得很直接：**達成因應調整的工具是診斷** | ❌ **它不知道你已經會什麼。** 它每次都給你滿級的成品 |
| **逐步撤除** | 支持隨時間遞減，而**遞減的速度由學習者的程度決定** | ❌ 它不會自己淡出。愈用只會愈順手 |
| **責任移轉** | 透過因應式的撤除，任務的責任逐漸交回學習者 | ❌ 沒有交還的機制。你不主動要，它不會還 |

三格全部落空。所以嚴格講，**你的技能檔預設不是鷹架，它是一棟隨時可以整棟搬過來的成品建築。** 要讓它變成鷹架，那三件事得由你自己從外面掛上去。

#### 1976 年就寫下的那個條件

「鷹架」這個詞出自 Wood、Bruner 與 Ross 1976 年的一篇觀察研究，他們看的是一位大人怎麼教三十個三到五歲的孩子搭積木。那篇文章裡有一句話， **幾乎沒有人引用，但它是這本書從第一章到最後一章的同一句話換個說法**：

> **理解解答必須先於產出解答。** 學習者必須先能**認出**某一類問題的正確解，才可能在沒有協助的情況下自己產出通往它的步驟。

原文把這個條件標為 **paramount**——**至高的**。意思是：這一條不成立的時候，協助**不會**轉化成能力。不是效果打折，是不會發生。

現在把它跟本書的判準並排：

| 1976 年的說法 | 本書的說法 |
|---|---|
| 你要先認得出正確的解 | **如果它做錯了，我看得出來嗎？** |
| 認不出來，協助不會變成能力 | 看不出來的地方交出去，錯誤會安靜地留在你的論文裡 |

⚠️ **同一篇文章還寫下了鷹架的失敗模式。** 六個鷹架功能裡的第五個是「挫折控制」，底下有一句：**最大的風險在於造成對教導者的過度依賴。** 提出這個概念的人，在提出它的同一頁就寫了這句話。而後續四十年的應用文獻，多半只引前四項。

而這推出一個不太舒服的結論：**最需要鷹架的人，最沒有能力把工具當鷹架用**—— 因為調整支撐量本身就需要判斷力，而那正是初學者最缺的東西。這不是叫你別用，是叫你**把撤除寫下來**。

#### 不是所有支持都一樣好，有一種會讓學習變差

前面講的是**什麼時候**撤。還有一個更前面的問題：**你要它給哪一種支持。**

Sasse 等人（2025）在德國國小做了一個準實驗，164 名 8–11 歲學生。他們把教師在課堂上臨機給的支持分成三類（分類沿用 Hsin 與 Wu 2011）：

| 類型 | 實際做了什麼 | 對學習的效果 |
|---|---|---|
| **促進參與** | 讓學生留在任務上、維持投入 | 沒有效果 |
| **聚焦** | **給個別建議、給策略提示、指出錯誤、鼓勵去看提示卡** | ❌ **負向** |
| **問題化** | 拋出讓學生自己想的問題，製造認知衝突 | ✅ 正向 |

看中間那一列的操作定義：**給建議、給提示、指出錯誤。**

那就是你不下任何指示時，工具的預設行為。

作者自己也覺得意外，寫明這與許多既有研究相反。他們的解釋是：那些「聚焦」多半給了還沒建立基礎的學生，而**太多結構反而讓人無法主動投入任務**。

⚠️ **這一條把本書那些 ⛔ 行的理由講明白了。** 回頭看你到目前為止貼過的每一段提示詞，裡面的「不要幫我改寫」「不要替我判斷」「不要給我答案，只出題考我」——**那些全部是在把工具從「聚焦」推向「問題化」。** 它們不是為了謹慎才加的，是為了讓那個支持真的產生作用。

〔動手一下〕**5 分鐘。** 打開你這一章建的技能檔，數一數：裡面有幾句是在要它**給你東西**，幾句是在要它**問你問題**？兩邊差太多的話，你知道要往哪邊補。

⚠️ **這一筆的限制要一起帶**：國小 8–11 歲、自然科探究課、面對面、由一位受過訓練的實驗者授課而非原班教師。**它沒有測過任何工具。** 把它讀成「AI 給建議會害你」是過度延伸；讀成「**支持的類型會決定效果的方向**」才是它說的。

#### 一段可以直接貼進去的〈退場〉

從現在起，你建的每一個技能檔，最下面多一節：

```text
## 退場

- 使用次數：第 5 次之後，先自己做一遍，再開這個檔案對照
- 降級動作：拿掉〈步驟〉那一節，只留〈品質標準〉當檢查清單
- 停用條件：連續三次我的版本跟它的沒有實質差異
- 上次裸做：____（空白就是警訊）
```

四行，寫完不用五分鐘。**回頭把它加到你已經建好的那幾個檔案，之後每建一個就寫一次。**

最後一行最重要，也最容易被跳過。它是一個你會定期看到的空格—— 而空格比任何提醒都有效。
...
| 你現在的狀態 | 動作 |
|---|---|
| 你看得出它哪裡不對，**而且說得出為什麼** | **降一級** |
| 你只覺得「怪怪的」，講不出來 | 停在原級，再跑幾次 |
| 你完全接受它給的 | ⚠️ **你在這一級待太久了** |

⚠️ **撤太晚會出事，撤太早也會。** van de Pol 等人（2019）觀察了 35 節課、一萬三千多個師生發話回合，發現學生對支持的採納**有時**正是被**不合時宜的撤除**妨礙的；而先因應、再逐步撤除的支持最有效。

⚠️ **但這一筆的證據強度要講清楚，因為這本書自己在教這件事。** 那句話出自該研究的**額外質性探索分析**，不是它的主要結果—— 它的主要量化結果反而是一個 null：**支持給得準不準，並沒有影響學生用不用它。** 所以上面那句話是**線索不是定論**，這裡採用它是因為它與撤除的邏輯相符，不是因為它已經被檢驗過。

不論如何，〈退場〉那一節寫的是**條件**不是日期——「連續三次沒有實質差異」是條件，「三個月後停用」不是。

〔動手一下〕**5 分鐘。** 打開你這一章建的那個技能檔，把上面那四行貼到最下面。「上次裸做」先填**空白**——那就是你現在的真實狀態。
...
- **刪規則是工具層，退場是能力層**，兩者要分開想
- 鷹架的三個成分——因應調整、逐步撤除、責任移轉——**你的工具三格全部落空**，得你自己掛上去
- 每個技能檔最下面加一節**〈退場〉**，四行：使用次數／降級動作／停用條件／**上次裸做**
- 退場寫的是**條件**不是日期
...
這一章有兩格要填——**因為這一章是你開始把做法固定下來的地方，而固定下來的東西最容易變成撤不掉的那一種。**

```text
本章的動作：把一段重複的做法寫成一個檔案
上次我完全不用那個檔案做同一件事：____________
上次我把它拿掉一段之後還做得出來：____________
```

第二行是 3.5〈也要寫退場〉那四行的縮版。你已經在技能檔裡留了同一格，這裡再留一次，是因為**章末你會看到，而技能檔要打開才看得到**。
...
- **Wood, D., Bruner, J. S., & Ross, G. (1976).** The role of tutoring in problem solving. *Journal of Child Psychology and Psychiatry, 17*(2), 89–100. [https://doi.org/10.1111/j.1469-7610.1976.tb00381.x](https://doi.org/10.1111/j.1469-7610.1976.tb00381.x) 「鷹架」這個詞的出處，本節的至高條件與依賴警告都出自它。**驗證層級：全文讀過。** ⚠️ 它被引超過五千次，但它是 **30 名兒童**的觀察研究，而且作者自己寫明「不應被視為對教導歷程的假設檢驗」。**引它建立概念恰當，引它證明效果則超出範圍**—— 這一句本身就是本書[第 5 章](../chapters/05-verification.md)要教的判讀方式。
- **van de Pol, J., Volman, M., & Beishuizen, J. (2010).** Scaffolding in teacher–student interaction: A decade of research. *Educational Psychology Review, 22*(3), 271–296. [https://doi.org/10.1007/s10648-010-9127-6](https://doi.org/10.1007/s10648-010-9127-6) 3.5 的〈也要寫退場〉那三個成分出自它，「用了支持策略不自動等於發生鷹架」那句話也是。 **開放取用。驗證層級：全文讀過。** ⚠️ **一個限制要一起知道，而且它比「這篇沒談 AI」更硬。** 那篇的文獻納入條件第六條明文寫著只收「**人的**鷹架，且為**面對面**接觸」—— 它是**從檢索階段就把非人的鷹架排除掉**，不是剛好沒收到。所以本書把三個成分拿來對照 AI，是**本書的延伸**，那篇對「工具能不能當鷹架」沒有任何主張，也沒有證據。順帶一提，它的效果結論只建立在 **8 篇**研究上，作者自己寫了「還需要更多研究」—— **引它來撐「鷹架有效」會超出它撐得住的強度**，那正是[第 5 章](../chapters/05-verification.md)在講的事。
- **Sasse, H., Weber, A. M., Reuter, T., & Leuchter, M. (2025).** Teacher guidance and on-the-fly scaffolding in primary school students' inquiry learning. *Science Education, 109*(2), 579–604. [https://doi.org/10.1002/sce.21921](https://doi.org/10.1002/sce.21921) 三類鷹架與「聚焦反而負向」出自它。**驗證層級：全文讀過。** ⚠️ 限制：國小 8–11 歲、自然科探究課、**由一位受訓實驗者授課而非原班教師**、只有兩堂課。**未測過任何工具。** 作者自陳六項限制，其中一項是只有一位實驗者、個人風格可能主導結果。
- **van de Pol, J., Mercer, N., & Volman, M. (2019).** Scaffolding student understanding in small-group work: Students' uptake of teacher support in subsequent small-group interaction. *Journal of the Learning Sciences, 28*(2), 205–239. [https://doi.org/10.1080/10508406.2018.1522258](https://doi.org/10.1080/10508406.2018.1522258) 「撤太早也會出事」出自它。**驗證層級：全文讀過。** ⚠️ **要連它的層級一起記住**：那句話來自**額外的質性探索分析**，而該研究的主要量化結果是一個 **null**——因應度並未影響學生採納或答對率。 ⚠️ 另外它的樣本是 **7 位教師、28 名學生**；一萬三千多個發話回合聽起來很大，但決定檢定力的是前兩個數字。**這正是[第 5 章](../chapters/05-verification.md)要你養成的讀法。**
