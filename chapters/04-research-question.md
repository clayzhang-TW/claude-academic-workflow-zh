...
**五題，三分鐘。** 這一章是全書最長的一章，值得先花三分鐘決定要讀哪些。

| # | 題目 | 過關的樣子 |
|---|---|---|
| **1** | 說出你那個技能檔在解決**哪一個摩擦** | 講得出一個你真的被絆到過的具體場景 |
| **2** | 它**已經有至少一條你事後加上去的規則** | ⚠️ 只建好、沒改過，等於還沒開始 |
| **3** | 描述對話變鈍的症狀，並說出三個應對**分別什麼時候用** | 三個都說得出使用時機，不是只記得有三個 |
| **4** | 你手上有**一個正在想或已經在做的研究題目** | 一句話講得出來，不必已經收斂 |
| **5** | 你找得出**一篇跟你的研究設計最像**（不是主題最像）的論文 | 你分得出「設計像」跟「主題像」的差別 |

前三題是硬前置。第 4 題缺了本章大半做不動，第 5 題缺了 4.6 做不動—— 但那兩題**現在答不出來也不必回頭**，它們正是這一章要幫你處理的東西。

**怎麼分流**

| 過幾題 | 這一章你該怎麼讀 |
|---|---|
| **5 題** | 挑著讀。4.1（收斂問題）與 4.6（拆解設計）是本章不可略的兩節，其餘看〈帶走這一節〉 |
| **3–4 題** | 正常讀。缺第 4 或第 5 題的人，4.1 與 4.6 要讀慢一點 |
| **2 題以下** | ⚠️ **先回[第 3 章](../chapters/03-skills.md)**。缺第 2 題最要緊——**沒有改過的技能檔，本章講的每一種固化都會讀起來像多此一舉** |
...
〔教你的 Claude 建這個〕**把追問變成預設。**

**先自己寫。** 這一份只有一條主規則加三條限制。主規則要回答： **什麼情況下它該先停下來問你，而不是直接開始做。**

⚠️ 難的是那個門檻。「重要的任務」判斷不了；你要給它一個**它自己估得出來**的標準。

**寫完之後，用這三條檢查：**

| 檢查 | 為什麼 |
|---|---|
| 你的門檻它**估得出來嗎** | 一個可用的寫法是「做錯要重做超過十分鐘」——它估得出重做成本，估不出「重不重要」 |
| 有沒有「**不要問我已經在指令裡講過的事**」 | 少了它，追問會變成把你的指令覆述一遍再問你一次 |
| 前提本身有問題時（樣本數不夠、方法配不上問題），它要**現在講**還是做完再講 | ⚠️ **這一條最容易被略過，而它擋掉的是最貴的那一種浪費**——整件事做完了才發現方向錯 |

**卡住超過十五分鐘再翻 → 〈範本對照冊〉第 8 則。**
...
〔教你的 Claude 建這個〕**把「定位」跟「反方」固定成一個檔案，把「回答」擋在外面。**

⚠️ **這一份你一定要自己寫，理由就是這一節的主旨。** 一份你照抄來的「拆解助手」，會很稱職地替你拆解——而這一節從頭到尾在防的就是那件事。

**先自己寫。** 只要交代三件事：**它可以做什麼、它不可以做什麼、以及每次回答固定要附什麼。** 難的不是前兩項，是想清楚**界線畫在哪裡**。

**寫完之後，用這三條檢查：**

| 檢查 | 為什麼 |
|---|---|
| 「可以做」那一欄，有沒有超出**定位**與**反方** | 指出方法寫在第幾節＝定位；列出替代解釋＝反方。多出來的每一項都在替你回答 |
| **有沒有一行 ⛔** | ⚠️ **那一行是這個檔案的全部價值。** 少了它，你會拿到一份完整的設計分析，然後你會照抄——這一節從頭到尾在防這件事 |
| 每次回答的最後，它要固定交代什麼 | 一個很省事的寫法：**上面哪些是你從文本讀到的，哪些是你推測的**。它會逼出你最需要警覺的那幾行 |

⚠️ **順序也要寫進去**：你自己讀完、把判斷貼給它，**那時候**它才挑毛病。反過來就是這一節警告的那件事——在你還不知道答案的地方要它給答案。

**卡住超過十五分鐘再翻 → 〈範本對照冊〉第 9 則。** ⚠️ 但這一則翻之前先問自己：**我是卡住，還是想省下自己想「該擋什麼」的那十分鐘？**
...
```text
本章的動作：從別人的論文裡讀出「它為什麼這樣設計」
上次我完全不用它做同一件事：____________
```

⚠️ **這一章的那一格特別要盯著，因為 4.6 是全書唯一一節明寫「不能交出去」的。** 如果這一格填不出日期，代表你正在把一件書裡標了不可委派的事委派出去—— 而那正是「錯誤沒有對照組」的那一格。
...
![圖 4-6 你在整條工作流的哪裡](../resources/figures/fig4-6_工作流地圖_W4.svg)

〔圖 4-6〕亮起來的是這一章教的位置，淡掉的是你在別章會站上去的地方。四章走完，起點與「蒐集」整段都亮了。但注意「查證」還是暗的。你手上這批文獻，還沒有一筆通過驗證。
