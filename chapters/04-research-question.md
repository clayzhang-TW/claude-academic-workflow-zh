...
- **操作**「PDF → 結構化筆記 → 知識庫」的完整流程，並修正 AI 填錯的欄位
- **區分**四種筆記的功能與時機
...
〔補充〕**還有兩種情境不走這一節的流程。**（一）**系統性回顧或範疇回顧**：它的要求不是「讀得快」，是**檢索過程可重現**——你得留下檢索式、資料庫、日期、去重規則與納入排除的每一筆判定。工具能幫的是跨庫檢索、去重與整理成可篩選的清單，幫不了的是納入排除判準，那必須你自己訂並且事先寫死。（二）**要啃一整本英文專書**：逐段中英對照比全文翻譯可靠得多，因為你隨時看得到原文那一句長什麼樣；全文翻譯會讓你在不知不覺中接受譯者的詮釋。兩種情境都不是「更進階的版本」，它們是**不同的工作**，用這一節的流程去做只會做壞。
...
| 二 · 流程 | 跑完之後**下一句修正**，看它改不改得對 | 第 5 欄具體到可以直接用在你的論文裡，不是「本研究與該主題相關」 | 跑完就收工。跑完只是開始，**糾正迴圈才是這個實作在練的東西**，而且真實使用中每一篇都要做 |
...
實作二與實作三的差別值得點出來：流程產出的是文獻筆記，那是**濃縮**，AI 做得很好；概念筆記是**你的理解**，AI 產不出來。同一篇論文跑完流程之後，概念筆記那一步還是得你自己寫。
...
- **第 3 章**：把重複的做法固定下來。第 2 章 2.1 的七欄流程正是把它固定成技能的最好題材
