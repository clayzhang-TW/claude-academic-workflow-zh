...
**這五題不是考試，是分流。** 花三分鐘做完，你會知道這一章要怎麼讀。

不要憑感覺答「應該會吧」——每一題都問**做得出來**或**說得出來**，所以答案只有「現在就做給你看」跟「還不行」兩種。

| # | 題目 | 過關的樣子 |
|---|---|---|
| **1** | 不看書說出**專案記憶與知識記憶差在哪** | 你講得出兩者的**生命週期**不同：一個做完就凍結，一個跟著你走 |
| **2** | 打開你自己的知識庫，裡面**已經有你放進去的東西** | 不是空的，也不是只有一個範例檔 |
| **3** | 說出**為什麼一個資料夾不夠** | 你舉得出一份東西同時屬於三個地方的例子 |
| **4** | 說出上一章那個**最小循環**的三個動作 | 貼進去／要它做一件具體的事／**把結果存成檔案** |
| **5** | 你手上有一篇**你已經讀過**的論文 PDF | 讀過，不是下載過 |

前三題是硬前置——缺了會卡住。後兩題缺了還走得動，只是實作要多花時間。

**怎麼分流**

| 過幾題 | 這一章你該怎麼讀 |
|---|---|
| **5 題** | 正文略讀。直接看各節的〈帶走這一節〉與 2.7 動手實作，卡住再回頭翻 |
| **3–4 題** | 正常讀 |
| **2 題以下** | ⚠️ **先回[第 1 章](../chapters/01-setup.md)**。缺第 1 題回 [1.2](../chapters/01-setup.md#12-你剛才那段對話明天就不見了)，缺第 2 或第 3 題回 [1.3](../chapters/01-setup.md#13-建你的現場)。本章第一頁就要往知識庫裡寫檔案，庫是空的就走不動 |

⚠️ **第一檔不是客套話。** 這一章的價值有一半在 2.2 那個「讓它騙你一次」的實驗，而那件事不需要你先讀完前面的正文。**已經會的部分不必再讀一遍，省下的時間拿去做實驗。**
...
〔動手一下〕**讀完之後，讓它考你。** 挑一篇你剛讀完的論文，貼進去，說：「針對這篇出五題問答，考我讀懂了沒。**不要給我答案。** 題目要包含：它的主要宣稱、它用什麼設計、它自己承認哪一個限制。」然後閉卷回答，答不出來的那幾題再回去讀那幾段。

⚠️ **注意這個用法跟「請它幫我讀」的差別，那個差別就是整本書的判準。** 你已經讀過了，所以它出錯題你看得出來；你沒讀就要它摘要，它錯了你看不出來。 **同一個工具，從輸入端換到驗收端，性質完全不同。** 如果你發現自己每篇都在用它，那就是[第 3 章](../chapters/03-skills.md)說的「該固定下來」的訊號。

〔教你的 Claude 建這個〕**把這五題固定成一個分流動作。**

**先自己寫。** 打開一個新檔案，用你自己的話交代三件事：

1. **輸入是什麼**（提示：不是整篇論文）
2. **要它依序回答哪五題**，以及每一題答成什麼樣才算數
3. **它不確定的時候該怎麼辦**
...
| 檢查 | 為什麼 |
|---|---|
| 第四題有沒有要求它說**為什麼**有關聯 | 少了這一句，你會拿到一整排「相關」，等於沒分流 |
| 有沒有一條處理**不確定**的規則 | ⚠️ **沒有它，你會拿到一份每篇都值得精讀的清單**——它傾向給你明確答案 |
| 輸入寫的是摘要與引言，還是整篇 | 整篇丟進去，這道閘門就變成了它要擋的那件事 |

三條都過，你的版本就可以用了。**卡住超過十五分鐘再翻 → 〈範本對照冊〉第 1 則。**
...
> **〔補充〕這七欄你自己也填得出來。** 它們不是為了讓 AI 有事做而設計的，它們是一份**閱讀提綱**—— 一篇論文該被問到的七件事。額度用完、斷網、或者你就是想自己讀一次的時候，打開一個空白檔案照著這七欄填，一樣填得完，只是慢。 ⚠️ **而且慢的那一次通常填得比較好**，因為第 5 欄（它推的是你的問題意識）本來就是你要改的那一欄。[第 7 章](../chapters/07-revision-integrity.md) [7.5](../chapters/07-revision-integrity.md#75-帶著什麼離開) 有整條流程的清點。
...
〔教你的 Claude 建這個〕**把這兩個標記變成你的預設。**

**先自己寫。** 這一段要交代的是「之後不必再重複講的規則」，所以它比上一個檔案短——大概三到四行就夠。你要寫進去的是：

1. 兩個標記各在什麼情況下出現
2. 一條關於**不確定**的規則
3. **一句驗收**——讓你事後看得出它有沒有真的照做

**寫完之後，用這三條檢查：**

| 檢查 | 為什麼 |
|---|---|
| `[CITATION]` 那一條有沒有寫「**不產生任何書目**」 | 少了它，它會很樂意幫你補一筆看起來很像的 |
| 有沒有一條處理不確定 | 沒有的話，它會在該說「不知道」的地方給你一個順暢的答案 |
| **驗收那一句是可數的嗎** | ⚠️ 「請確實遵守」不是驗收。**要它在結尾報出各放了幾個**——數字對不上，你才看得出它漏標了 |

第三條是這個檔案唯一難想到的一條，也是它全部的價值。 **卡住超過十五分鐘再翻 → 〈範本對照冊〉第 2 則。**
...
留一格給自己填，現在填不出來沒關係——**這一格的功能是讓你以後看得到它**。

```text
本章的動作：把一篇論文讀成一份結構化筆記
上次我完全不用它做這件事：____________
```

⚠️ **不是叫你不要用。** 這七欄是一份閱讀提綱，你自己填得出來，只是慢；而慢的那一次通常填得比較好（2.1 講過）。這一格要防的是另一件事：**填不出日期而你沒發現。**

[第 3 章](../chapters/03-skills.md) [3.5](../chapters/03-skills.md#35-讓它長大) 會把這一格變成你每個工具檔案的固定結尾。
