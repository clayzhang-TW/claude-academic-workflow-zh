...
**開始前請準備**：一份你自己的統計輸出（任何軟體、任何分析都行）；你常翻的一本方法論專書或其中一章；[第 5 章](../chapters/05-verification.md)歸好的容器內容。

**你現在在哪**

**五題，三分鐘。** ⚠️ 這一章是全書**出錯代價最高**的一章，所以第 2 題答不出來的人，請務必回去補。

| # | 題目 | 過關的樣子 |
|---|---|---|
| **1** | 拿出一組查證過、標了驗證層級的筆記 | 至少五筆，而且驗證層級是你標的 |
| **2** | 說出四步核實**抓不到**什麼 | ⚠️ 「文獻存在 ≠ 你引的那句話正確」——答不出來的人會過度信任自己手上的材料 |
| **3** | 你手上有**一份自己的統計輸出** | 任何軟體、任何分析都行，但要是你自己跑的 |
| **4** | 說出「語法錯」與「模型選錯」哪一種會報錯 | 你講得出「一種會當著你的面出聲，一種不會」 |
| **5** | 你常翻的方法論專書，說得出是哪一本 | 說得出書名，而且你真的翻過 |

第 1、2 題是硬前置。第 3 題缺了本章前半只能看示範。第 5 題缺了 6.2 做不動，但那一節本來就是給「遇到不會的方法」的人。

**怎麼分流**

| 過幾題 | 這一章你該怎麼讀 |
|---|---|
| **5 題** | 6.1〈它跑完了不代表跑對了〉不可略，其餘看〈帶走這一節〉 |
| **3–4 題** | 正常讀 |
| **2 題以下** | ⚠️ **先回[第 5 章](../chapters/05-verification.md)**。缺第 2 題最要緊——這一章的錯誤最不容易浮出水面，而你會拿它去撐論文裡的數字 |

---

## 章前導言

[第 5 章](../chapters/05-verification.md)結束的時候，你手上有三句主張和歸好隊的證據。

接下來有兩件事要做，而**它們的順序不能顛倒**：先讓資料說話，再把它寫成字。

很多人其實是反過來做的。這跟懂不懂沒關係，問題在於寫作有一種假的進度感：你在文件裡打了兩千字，看得見、算得出來；而分析跑不出來的時候，螢幕上什麼都沒有。於是人會傾向先去寫那個看得見進度的部分，包括在分析還沒收斂的時候就開始寫討論。
...
**先講清楚本章的邊界。** 本章不教統計方法。哪個方法適合你的資料、前提假設成不成立、效果量怎麼解讀，那是方法論課程與你的指導教授的事。本章教的是當你已經知道要用什麼方法時，工具能替你做到哪裡，以及界線畫在哪。而 6.2 處理的是另一種處境：你連方法都還不熟的時候，怎麼建一個問得下去的對象。
...
這件事的形狀，你其實已經很熟：**它比較像你委託人代抄一份編碼簿。** 對方把欄位名稱抄錯了，你一匯進 SPSS 就會發現讀不進去；但對方抄錯的如果是「這一題該正向計分還是反向計分」，資料照樣讀得進去，平均數照樣跑得出來，而且整份報表看起來完全正常。 **兩種錯的差別不在難度，在於哪一種會當著你的面出聲。**
...
所以「幾乎不會錯」講的是會出聲的那一半，而那一半本來就不需要你太操心。不會出聲的那一半，正確率再高也沒有意義——**因為你沒有辦法知道這一次是不是那個例外。**
...
〔教你的 Claude 建這個〕把你的程式慣例寫成一份規格檔，這樣你就不必每次重講。

統計協作最惱人的不是它寫不出來，是**它每次寫出來的風格都不一樣**—— 這次用一種寫法，下次用另一種，同一份分析檔看起來像三個人寫的。

**先自己寫。** 這一份跟前面幾份不同：它要問的東西你**已經知道答案**，只是從來沒寫下來過。所以它應該由**五個問題**開始。

**寫完之後，用這三條檢查：**

| 檢查 | 為什麼 |
|---|---|
| 五個問題涵蓋了語言與套件、**變數命名**、數值報告格式、顯著性標記、產出去向了嗎 | 少哪一項，那一項就會每次都不一樣 |
| **顯著性那一題有沒有問「有沒有你明確不用的」** | 例如邊際顯著。⚠️ **不寫，它就會照它學到的慣例給你**，而那不見得是你領域的慣例 |
| 每一條規格**寫成可以檢查的形式**了嗎 | 「程式碼要清楚易讀」判斷不了對錯。⚠️ 這一條跟 [3.4](../chapters/03-skills.md#34-提示詞技能代理) 的規則檔是同一句話 |

**卡住超過十五分鐘再翻 → 〈範本對照冊〉第 14 則。**
...
〔補充〕**貼報表給 AI 的時候，貼的是統計結果，不是原始資料。** 平均數、係數、p 值、配適指標，這些沒有個資問題。含個案資料的原始資料列不要貼。如果你需要它看資料結構，貼變項清單，或去識別化的前三列就夠了。這一條在[第 7 章](../chapters/07-revision-integrity.md) [7.4](../chapters/07-revision-integrity.md#74-誠信的完整框架) 會再展開。
...
6.1 講了一條界線：語法錯會報錯，模型選錯不會。這裡要講一件更深的事：**模型選對了，跑完了，結果也出來了，但那組結果是假的。**

這叫**數值病態**。一句話講完它是什麼：**程式順利收斂了，但它交給你的那組估計值在統計上站不住。**

它的形狀，你在別的地方見過。**想像一道所有人都答對的題目。** 通過率 1.00，看起來漂亮；而它的鑑別度是零——它跟總分之間算不出任何關係，因為它根本沒有把任何人分開。你不會說這題很好，你會說這題沒在測東西。

**退化解裡那個殘差趨近零的組，就是模型裡那道所有人都答對的題目。** 它讓整體的配適看起來很好，正是因為它本身不含任何需要被解釋的東西。而好看，是你唯一收到的訊號。

![圖 6-2 為什麼「配適變好」有時候是壞消息](../resources/figures/fig6-2_無變異讓指標好看.svg)

〔圖 6-2〕上下兩排是同一個機制的兩個場合。 **要看的是點的排法**：左邊散得開，右邊全部擠在同一格。擠在同一格的那一欄，指標都是最好看的那一個，而它們什麼都沒有分開。

數值病態在混合模型（潛在類別、成長混合模型、潛在剖面分析）裡特別常見，它長這樣：

| 訊號 | 長什麼樣 | 為什麼危險 |
|---|---|---|
| **退化解** | 某一組的殘差趨近零，或人數只有個位數 | 程式跑完不報錯，配適指標甚至變好——因為一個幾乎沒有變異的組當然配適得好 |
| **邊界解** | 變異數估計碰到零的邊界（Heywood case），或相關矩陣不正定 | 有些軟體會印警告，有些不會。不印的那些就是 6.1 講的靜默失敗 |
| **局部極大值** | 每次跑出來的結果不一樣，換一組起始值就換一組答案 | 你永遠只看到你碰巧跑出來的那一組。你不會知道還有另一組，除非你主動用多組隨機起始值去測 |

三種病態各對應上面表格的一列。退化解：某一組的殘差趨近零，配適指標顯示「最佳解」但在統計上根本不可解讀——因為一個幾乎沒有變異的組可以吸收任何殘差。邊界解：變異數估計卡在零上，表面上收斂了，要到你去查參數表才發現有一列不對。局部極大值：只跑一批起始值就定案，換一批之後類別結構完全不同——你先前報告的那個解，只是碰巧先到的那一個。
...
![圖 6-3 收斂之後，還有三個檢查](../resources/figures/fig6-3_收斂之後的三個檢查.svg)

〔圖 6-3〕紅色虛線是程式世界的盡頭。**線以上是軟體會做的，線以下沒有任何東西會提醒你。** 三個檢查全部落在線以下。

**一個判準**：如果你用的是任何「把人分成幾組」的方法，跑完之後問自己三件事——

1. **最小那一組有幾個人？** 小於樣本數的 5% 就要懷疑
2. **換一批隨機起始值，答案一不一樣？** 不一樣就是局部極大值
3. **有沒有任何一個參數估計碰到邊界？** 變異數 ≈ 0、相關 ≈ ±1 都是

三題有任何一題不過，那組結果**不能直接報告**。它需要的不是更多分析，是回頭重新檢查模型設定。

⚠️ **AI 不會替你做這三個檢查。** 你問它「這個結果合理嗎」，它會看配適指標，而配適指標恰好是退化解看起來最漂亮的地方。這是 6.1 那張圖的第六行加粗的原因：選模型、判資料品質、撐多強的主張——判斷層的三件事，全部落在你這一條泳道上。
...
有一個做法可以解決這件事，而且它同時是這本書從[第 3 章](../chapters/03-skills.md)開始鋪的那條暗線的收束點。

### 把一本書變成一個顧問

[第 3 章](../chapters/03-skills.md)教你建立自己的技能。[第 4 章](../chapters/04-research-question.md)你用它跑文獻流程。本章 6.3 你會用它寫作。這些都是別人設計好的工具。
...
〔教你的 Claude 建這個〕**萃取結構，不要摘要。**

這是整個轉換成敗的分水嶺，所以它必須寫進指令裡，不能靠期待。

**先自己寫。** 你要交代的是**五類東西**與**一個順序**。 ⚠️ 難的不是列出五類，是想清楚**為什麼不能讓它先生成檔案**。

**寫完之後，用這三條檢查：**

| 檢查 | 為什麼 |
|---|---|
| 開頭寫的是「**萃取結構，不要寫摘要**」嗎 | 摘要你讀完就忘，結構你三個月後查得到。這是分水嶺那一句 |
| 框架的名字要不要**原樣保留** | ⚠️ **不寫這一條，它會換成它覺得比較好懂的同義詞**——而你日後拿那個詞去查文獻會查不到。這是[第 5 章](../chapters/05-verification.md)術語紀律的同一件事 |
| **它是先給你清單，還是直接生成檔案** | 先只做分析：偵測到哪些框架、分佈在哪幾章、建議切幾個檔案。你確認之後再生成——⚠️ **直接生成的話，你拿到的是它對整本書的理解，而你沒有機會發現它漏了哪一章** |

五類是：有名字的框架／可行動的判斷規則（門檻、樣本數、何時可放寬）／步驟方法／反面樣式（作者說不要做什麼，以及為什麼）／作者的判斷傾向。

**卡住超過十五分鐘再翻 → 〈範本對照冊〉第 15 則。**
...
〔補充〕**這不會取代你讀書。** 你仍然需要理解那個方法在做什麼，否則你連問對問題都做不到。它取代的是「每次都要翻回去查那本書第 8 章第 3 節的那張表」這件事。判斷是你的，查表是它的。這正是本書從[第 1 章](../chapters/01-setup.md)講到現在的同一條界線。

### 這就是「工具會成長」的意思

回頭看你走過的路。

[第 3 章](../chapters/03-skills.md)你從一個摩擦點出發，寫了一個只有〈使用者背景〉的技能。[第 4 章](../chapters/04-research-question.md)你把文獻筆記的七欄規格加了進去，並發現有個欄位老是抓錯方向，你又改了一次。本章 6.3 你在寫作的時候發現某個格式要求每次都要重講，你也把它寫進去。

現在你把一本書變成了一個技能。

這四步是同一件事的四個階段：**你的工具正在往你的方向長。** 而它從第一天起就是你的，因為它的每一段都源自你自己遇到的摩擦，不是從別人那裡繼承來的。

這一點值得停一下。如果[第 3 章](../chapters/03-skills.md)你抄了一個現成的技能來改，你現在擁有的會是一個「大致堪用但你說不清楚為什麼那樣設計」的東西。而你自己長出來的這個，每一行你都知道當初為什麼加。**能不能說出為什麼，決定你未來能不能判斷它該不該改。**
...
**第一個是角色分工。** [第 3 章](../chapters/03-skills.md)教你指定 AI 的表現（Performance），成熟版是三個利害衝突的角色接力：一個負責找出可用的來源，一個負責逐筆質疑並在打掉之後給替代方案，第三個只看跨筆的一致性（同一個主張有沒有前後用了兩種強度）。三者的動機是衝突的，正因為衝突才互相抓得到。 ⚠️ 最小可用版只要兩個角色，但**不要在同一次對話裡**：同一次對話裡它記得自己剛寫了什麼，於是質疑會客氣。

**第二個是人的關卡。** [第 4 章](../chapters/04-research-question.md) [4.2](../chapters/04-research-question.md#42-該不該交給它) 那張風險表（可偵測性 × 不可逆性）在成熟工作流裡變成流程中途的強制停點。我的審稿回應工具有三個關卡，全部落在同一種位置：這一步做下去就難回頭。其中一個關卡帶著一條硬規則。補跑的統計若結果不利於原結論，不得靜默不報。那條規則不是禮貌，是把誠信寫成機制。

**第三個是迴圈上限。** 本書[第 7 章](../chapters/07-revision-integrity.md)的修稿三輪，其實就是這個模式：迴圈沒有上限就會一直改，而每一輪的邊際收益遞減、風險遞增。改久了你會把原本對的地方改壞。
...
[第 5 章](../chapters/05-verification.md) [5.3](../chapters/05-verification.md#53-從二十篇筆記到一個論點) 處理的是寫作之前的事。現在你有了三句主張（或兩句，或被 AI 修正過的四句），下一步是把它們寫成論文段落。
...
![圖 6-4 寫作模式鏈](../resources/figures/fig6-4_寫作模式鏈.svg)

〔圖 6-4〕六個階段攤成一條流程，右下角三條是使用規則。 **draft 那一格用紅框標出來**，因為它是唯一一個「它可能替你生出東西」的階段—— 下一小節整節在講那件事。
...
![圖 6-5 整理過的那份，不是拿來取代原稿的](../resources/figures/fig6-5_傾倒稿是唯一的對照組.svg)

〔圖 6-5〕去程一條、回程一條。**沒有那條回程線，左邊那一團就白倒了**—— 而回程線之所以容易被省掉，正是因為右邊那份讀起來已經很好。
...
〔補充〕**這個做法我是二手讀到的**，出自寫作教學學者 Rowena Murray，我沒有讀過她的原書。照[第 5 章](../chapters/05-verification.md)的規矩，這一句只能撐到「有這樣一個做法」為止；你要把它寫進論文或教案並引用她，得自己去讀原書。本書〈出處〉頁記了我從哪裡讀到它。

這一段跟[第 7 章](../chapters/07-revision-integrity.md) [7.1](../chapters/07-revision-integrity.md#71-修稿三輪) 的**逆向大綱**是同一件事的兩端：這裡是動筆之前先訂結構，那裡是寫完之後把實際寫出來的結構重新抽出來對照。兩邊都做，你會很快看出自己是在哪一節寫著寫著跑掉的。

### 主動餵文獻，不是等它自己找

寫作模式鏈裡有一個設計，如果你不理解它的原因，你就不會照做。

當 AI 在 draft 模式寫到一段需要引用文獻的地方，如果你沒有在對話中提供那篇文獻的筆記或內容，它不會自己去找一個「看起來合理」的引用（那正是[第 5 章](../chapters/05-verification.md)講的引用幻覺的產生方式）。正常設計的寫作工具會標記 `[MATERIAL GAP]`，告訴你「這裡需要文獻支持，但你沒給我素材」。

這不是工具的缺陷，是刻意的設計。它的邏輯是[第 1 章](../chapters/01-setup.md)就講過的：AI 擅長的是把你給它的東西重新組合，不擅長的是從零生成事實。所以正確的做法是**你主動餵文獻**，而不是期待它自己找。

但這個標記有一個限制，而且它直接來自[第 1 章](../chapters/01-setup.md)的結論：**它無法可靠地區分自己知道與不知道。** 所以標記抓得到的，是它意識到「這裡需要引用而我沒有素材」的地方；抓不到的，是它以為手上的素材夠、實際上那段素材撐不起那個主張的地方。

判準因此是不對稱的：**標記出現，你一定要處理；標記沒出現，不代表沒問題。** 這跟[第 2 章](../chapters/02-first-note.md)的結論是同一件事，驗證那一步不會因為工具設計得好而省掉。

而你能餵它什麼？正是[第 4 章](../chapters/04-research-question.md)建的那個知識庫。

流程是這樣的：你在 draft 模式寫到一段關於「同儕回饋的效果」的論述，你知道 [5.3](../chapters/05-verification.md#53-從二十篇筆記到一個論點) 步驟二的容器裡有哪些證據片段可以支持這段。你把那些片段（連同來源論文的筆記）貼進對話中，然後讓 AI 在這些素材的基礎上幫你組織段落。它能做的是調整表述、連接段落、處理引用格式；你做的是決定哪些證據用、用在哪裡、支持多強的主張。
...
〔教你的 Claude 建這個〕把模式做成一個路由表，而不是六個檔案。

六種模式各建一個檔案是很自然的想法，**但那會讓你每次都要先決定開哪一個**—— 而你多半是寫到一半才發現自己需要換模式。

**先自己寫。** 一個檔案裝六種模式，開頭一張路由表：**你說什麼、它切到哪一種**。六種是：規劃、大綱、草稿、修訂、格式、審查回應。

**寫完之後，用這三條檢查：**

| 檢查 | 為什麼 |
|---|---|
| 切換模式之前，它要**先告訴你**還是安靜地換 | ⚠️ **這一條防的是一個很難察覺的失敗**：你以為它還在幫你想結構，它其實已經在幫你潤詞了。讀起來越順，你越不會回頭檢查結構 |
| 草稿模式**只能用你提供的材料**嗎 | 需要引用但你沒給素材的地方，要它標記缺口 |
| 那個標記，你寫的是「標記缺口」還是「**不要自己補一個看起來合理的**」 | 前者是要求，後者是禁令。⚠️ **只寫前者，它會標記完再順手補一個** |

⚠️ 最後記得反過來問它一件事：**這六種裡你最常在哪一種卡住**—— 那一種要寫得比其他五種細。

**卡住超過十五分鐘再翻 → 〈範本對照冊〉第 16 則。**
...
指得回去還不夠，還要對得上強度。你跑的是相關分析，意涵就不能寫「應導入」；你的樣本是一所學校的兩個班，意涵就不能寫「建議全面實施」。這件事[第 5 章](../chapters/05-verification.md)講過一次（主張強度要匹配證據），在這裡它換一個位置又回來了。

〔動手一下〕**10 分鐘。** 找一篇你讀過的論文，只讀它的意涵那幾段。每一句問一次「指回哪個結果」，把指不回去的畫掉。多數論文畫完之後會剩下一到兩句——**那一兩句就是它真正主張的東西**，其餘是襯墊。你自己的稿子遲早會被同樣這樣讀一次。

**AI 在這一步的位置要特別小心。** 它產出通用意涵的能力極強，因為那正是它最擅長的東西：把大量論文的意涵段落平均起來，得到一段哪裡都適用的話。

⚠️ **而這一步命中全書判準的壞那一邊。** 問「如果它做錯了，我看得出來嗎」，答案是**看不出來**——一段通用意涵沒有事實錯誤、沒有引用問題、文法漂亮、結構完整，它唯一的問題是它跟你的資料沒有關係。你不會在校稿的時候抓到它，你的審查者會。
...
**第六週**，她跑修稿三輪（[第 7 章](../chapters/07-revision-integrity.md)），然後把計畫書交給指導教授。她在附件裡放了一份 AI 使用聲明，四類資訊都寫齊了。教授回信問的第一個問題是關於第三週那個方法轉換，因為那是計畫書裡最關鍵的判斷。
...
前兩個實作處理的是分析，這一個處理撰寫。用你在[第 5 章](../chapters/05-verification.md)歸好的容器，實際寫出一段。

**第一步：準備素材（5 分鐘）**

挑一句主張，把它的容器內容整理出來：三到五則證據片段，每一則附上來源筆記與驗證層級。

沒有做過[第 5 章](../chapters/05-verification.md)綜整的人，這一步會卡住。**卡住本身就是這個實作的第一個發現**—— 你會發現自己手上有的是「讀過的論文」，不是「歸好隊的證據」。
...
*為什麼錯*：draft 產出的是**形狀正確的段落**。有起承轉合、有連接詞、有學術語域。形狀正確會讓人低估要改的幅度，因為它讀起來不像草稿，讀起來像成品。真正要改的往往不在句子層，在哪些證據被放進來、放在哪個位置、撐了多強的主張，而那一層看起來完全沒問題。
...
```text
本章的動作：把一段分析從跑出來到寫成人看得懂的段落
上次我完全不用它做同一件事：____________
```

⚠️ **這一章的那一格最容易填不出來，而它的代價也最高。** 6.1 講過：語法錯會當著你的面出聲，模型選錯不會。 **而判斷模型選得對不對，靠的是你自己跑過、自己被結果嚇過的那幾次。**

那一格空太久，你會變成一個看得懂輸出、卻判斷不了輸出對不對的人—— 那正是這一章第一句話要防的事。
...
那位碩士生走到這一章，遇到的是同一個判準換了一個地方出現。他在 draft 模式寫到需要引用的段落，螢幕上跳出材料缺口的標記。**那個標記做的事，正是他[第 2 章](../chapters/02-first-note.md)缺的那一道**：它寧可告訴他少了素材，也不替他補一筆看起來合理的引用。差別在於這一次攔住他的不是指導教授的回信，是工具本身。

![圖 6-6 你在整條工作流的哪裡](../resources/figures/fig6-6_工作流地圖_W6.svg)

〔圖 6-6〕亮起來的是這一章教的位置，淡掉的是你在別章會站上去的地方。到這裡才動筆。圖上的順序就是本書的主張：**分析在撰寫之前**。先知道資料說了什麼，才有東西可寫。
...
2. 交換一段初稿，逐句問對方「這句主張的證據在哪」。幾句答得出來？
3. 統計協作裡哪一步你們願意交出去、哪一步不願意？兩人的界線畫在同一個地方嗎？

### 下章預告

初稿寫出來了。[第 7 章](../chapters/07-revision-integrity.md)處理它離開你的手之前的最後幾件事：**修稿、投稿、回應審查**。
...
- **Academic Research Skills**（Cheng-I Wu）：[https://github.com/Imbad0202/academic-research-skills](https://github.com/Imbad0202/academic-research-skills) 6.3 的六種模式與 `[MATERIAL GAP]` 標記出自這裡。它另外還有文獻探索與論文審查兩套，規模比本書用到的大很多。**授權 CC BY-NC 4.0**，非商業可自由使用與改編，請註明出處。 ⚠️ **它是給會操作技能檔的人用的**，不是入門教材。先讀完本書[第 3 章](../chapters/03-skills.md)再去看會比較順。驗證層級：讀過 README 與授權宣告，未逐一跑過它的四套技能。
- **Anthropic — Best practices for Claude Code**：[https://www.anthropic.com/engineering/claude-code-best-practices](https://www.anthropic.com/engineering/claude-code-best-practices) 雖然是寫給工程師的，但其中兩點跟本書完全重疊：**先研究再規劃、不要讓它直接動手**，以及多數最佳實務都源自同一個限制：脈絡會很快填滿，填滿之後品質下降。後者就是[第 3 章](../chapters/03-skills.md) [3.2](../chapters/03-skills.md#32-那個檔案裡該寫什麼)。

**本書其他章節**

- **[第 3 章](../chapters/03-skills.md) [3.2](../chapters/03-skills.md#32-那個檔案裡該寫什麼)**：脈絡管理。6.2 的「產物切塊、按需載入」是它的直接應用
- **[第 3 章](../chapters/03-skills.md) [3.4](../chapters/03-skills.md#34-提示詞技能代理)**：建你自己的技能。6.2 的方法顧問是它的最完整形式
- **[第 4 章](../chapters/04-research-question.md)**：文獻蒐集與知識庫。6.3 主動餵的素材全部來自那裡
- **[第 5 章](../chapters/05-verification.md) [5.3](../chapters/05-verification.md#53-從二十篇筆記到一個論點)**：綜整四步驟。6.3 餵進去的容器就是那一節歸好的
- **[第 7 章](../chapters/07-revision-integrity.md)**：修稿、投稿與審查。本章產出的初稿，下一章送它出門
- **[附錄 C](../appendix/B-E-toolkit-policy.md)**：Prompt 範本（含本章 6.5 實作三的骨架）
